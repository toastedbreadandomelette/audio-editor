...
// The parent should have the track resize movement bubbled up.
export class AudioTrackElement extends HTMLElement {
    ...
    _manipulationMode: AudioTrackManipulationMode = AudioTrackManipulationMode.None;
    ...
    private drawCanvas() {
        ...
        if (existingOffcanvas) {
            this.canvasContext.drawImage(
                existingOffcanvas, 0, 0, this._width, this._height - 20
            );
            return;
        }
        ...
        this.canvasContext.drawImage(
            offcanvas, 0, 0, this._width, this._height - 20
        );
    }
    ...
    private createTitleContainerElement() {
        ...
        titleContainer.style.backgroundColor = this._trackSource.trackDetail.selected ?
            'rgba(239, 68, 68)' : 
            this._trackSource.colorAnnotation;
        titleContainer.style.width = this._width + 'px';
        const waveElement = document.createElement('wave-icon');
        waveElement.w = 22;
        waveElement.h = 22;
        waveElement.s = '#fff';
        waveElement.vb = '0 0 2 2';
        titleContainer.appendChild(waveElement);  
    ...
    private onHoveringOverSelf(event: MouseEvent) {
        const {offsetX} = event;
        const {clientWidth, scrollLeft} = this;
        const cursorPosition = offsetX - scrollLeft;
    ...
    private initializeAudioTrack() {
        ...
        this.canvasElement.height = this._height - 20;
        this.classList.add(...TRACK_CONTAINER_CLASSLIST.split(' '));
        this.style.width = this._width + 'px';
        this.style.height = this._height + 'px';
        this.style.left = this.calculateLeft() + 'px';
        this.setAttribute('data-audioid', this._id.toString());
        this.setAttribute('data-trackid', this.trackID.toString());
        this.setAttribute('data-selected', this.trackSource.trackDetail.selected.toString());
